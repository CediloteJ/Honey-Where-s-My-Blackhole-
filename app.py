...
target_name = 'Sgr A*'
# Hard-coded ICRS coordinates for Sgr A* (RA 17h45m40.0409s, Dec -29d00m28.118s).
# NOTE: SkyCoord.from_name('Sgr A*') is deliberately NOT used here -- the asterisk
# in the name is a known trouble spot for the Sesame name resolver it queries over
# the network, and it can silently return coordinates for the wrong catalog object
# depending on the server/environment. Hard-coding avoids that ambiguity entirely
# and removes a network dependency from every request.
SGR_A = SkyCoord(ra=266.41681662 * u.deg, dec=-29.00781056 * u.deg, frame='icrs')
h_meters = 3.0
freq_hz = 20.1e6
c = 3.0e8
...
<body>
    ...
    <div class="card">
        ...
        <input type="number" step="0.01" id="latInput" value="34.20">
        <label for="lonInput">Lon:</label>
        <input type="number" step="0.01" id="lonInput" value="-79.76">
        <br><br>
        ...
        window.onload = function() {
            if (navigator.geolocation) {
                ...
                navigator.geolocation.getCurrentPosition(
                    ...
                    (err) => {
                        // Geolocation failed (denied, blocked on non-HTTPS host, timeout, etc).
                        // IMPORTANT: don't fail silently -- tell the user and leave the
                        // pre-filled fallback lat/lon in place so they can edit it manually.
                        document.getElementById('status').innerText =
                            `Could not detect location automatically (${err.message}). Using default Lat/Lon below -- edit them if that's not you.`;
                    },
                    { timeout: 8000 }
                );
            } else {
                document.getElementById('status').innerText = "Geolocation not supported by this browser. Enter Lat/Lon manually.";
            }
        };
